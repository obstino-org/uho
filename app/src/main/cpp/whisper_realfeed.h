...
//#endif
    ...
    whisper_mel getNextStreamMel(shared_ptr<MainStream> stream, bool& shouldResetPrompt, atomic<bool> &stopLooperThread, bool &dontPopBackExtra, double &outSilenceTime) {
        ...
        while(true && !stopLooperThread) {
            ...
            __android_log_print(ANDROID_LOG_INFO, "UHO3", "noSilenceNewBuffer size = %.3f seconds, silenceTime = %.2f", (double)noSilenceNewBuffer.size()/(double)config.sr, silenceTime);

            if((!noSilenceNewBuffer.empty() || prevBreakBecauseOfFramestep) && silenceTime >= 0.1) {
                __android_log_print(ANDROID_LOG_INFO, "UHO2", "BREAKING because of SHORT silence");
            ...
            if(silenceTime >= config.maxSilenceResetTime) {
                __android_log_print(ANDROID_LOG_INFO, "UHO2", "BREAKING because of LONG silence");
            ...
            if(noSilenceNewBuffer.size() >= frameStep) {
                __android_log_print(ANDROID_LOG_INFO, "UHO2", "BREAKING because of frameStep");
