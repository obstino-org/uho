...
public:
    ...
    Whisper()
    {
        ...
        */
        ...
        wprintf(L"Reading assets\n");
#if defined(TINY_MODEL)
        vector<uint8_t> encoderData = readAsset((char*)"tiny_encoder.onnx");
        vector<uint8_t> decoderData = readAsset(manager, (char*)"tiny_decoder.onnx");
#elif defined(CTX10_BASE_MODEL)
        vector<uint8_t> encoderData = readAsset((char*)"ctx10encoder.onnx");
        vector<uint8_t> decoderData = readAsset((char*)"ctx10decoder.onnx");
#elif defined(CTX6_BASE_MODEL)
        vector<uint8_t> encoderData = readAsset((char*)"ctx6encoder.onnx");
        vector<uint8_t> decoderData = readAsset((char*)"ctx6decoder.onnx");
#else
        vector<uint8_t> encoderData = readAsset((char*)"kv_encoder.onnx");
        vector<uint8_t> decoderData = readAsset((char*)"kv_decoder.onnx");
        ...
        vector<uint8_t> filtersVect = readAsset((char*)"filters_vocab_gen.bin");
        ...
        vector<uint8_t> vocabVect = readAsset((char*)"vocab.json");
    ...
    vector<uint8_t> readAsset(char* assetFileName) {
        FILE* f = fopen(assetFileName, "rb");
        fseek(f, 0, SEEK_END);
        long len = ftell(f);
        rewind(f);
    ...
    float computeTokenProbability(int tokenIndex, int tokenId, float* decoderOutput, float temperature)
    {
        ...
        //wprintf(L"decoderOutput[...]=%.2f, softmaxNumerator=%.2f, denominator=%.2f", (double)decoderOutput[tokenIndex * numTokens + tokenId], (double)softmaxNumerator, (double)softmaxDenominator);
    ...
    void updateBeamPromptProbabilities(shared_ptr<Whisper::BeamStructure> beam, vector<size_t>& prompt, shared_ptr<DecoderOutput> decOut) {
        ...
        for (int i = startIndex; i < numTokens - 1; i++) {
            ...
            //wprintf(L"Prompt probability is %.2f", prob);
        }
    }
    ...
    bool bestBeamCompressionRatioCheck(Whisper& whisper, vector<size_t>& prompt, shared_ptr<Whisper::BeamStructure> beam0, int& highComprIdx)
    {
        ...
        if (peakComprRatio >= maxComprRatio) {
            ...
            wprintf(L"Best beam compression ratio (%.2f) is poor. PeakStrIndex=%i\nText is \"%s\"\nNew text is \"%s\"\n", peakComprRatio, peakStrIndex, s1.c_str(), s2.c_str());
            return false;
        }
        return true;
    }
    ...
    int levenshteinDist(string word1, string word2) {
        ...
        vector<vector<int>> verif;  // TODO CHECK IF THIS IS CORRECT
        verif.resize(size1 + 1);
        for (vector<int>& v : verif)
            v.resize(size2 + 1);
    ...
    shared_ptr<Whisper::BeamStructure> additiveInference(whisper_mel& mel, vector<size_t>& prompt, int beamSize = 4, float temperature = 1.0f, int groupLogProb = 5, double groupLogP_thresh = -2.0)
    {
        ...
        wprintf(L"(NEW FOR PROMPT) prompt.size()=%i, probsPrompt.size()=%i", prompt.size(), beam->probsPrompt.size());
        ...
        while (prompt.size() >= groupSize) {
            ...
            if (groupLogP <= groupLogP_thresh) {
                wprintf(L"(NEW FOR PROMPT) Poping from prompt, groupLogP=%.2f < groupLogP_thresh=%.2f", groupLogP, groupLogP_thresh);
                prompt.erase(prompt.begin());
            }
            else {
                wprintf(L"(NEW FOR PROMPT) Not poping anymore");
                break;
            }
        }
        ...
        */
        ...
        for (int i = 0; i < maxSeqLen; i++) {
            ...
            if (suppressNextTime != 0) {
                wprintf(L"Yes we have to suppress token %i\n", suppressNextTime);
                for (int j = 0; j < beams.size(); j++) {
                ...
                (beams[0]->probs[beams[0]->probs.size() - 1] < 0.1 && !isFirstToken))    // end decoding if token probability is *really* low (and its not the first token)
            {
                ...
                //wprintf(L"ADIN DEBUG 4");

                /*if(tmpProb < 0.1)
                    wprintf(L"EXTREMELY LOW PROBABILITY TOKEN %i, breaking", tmp);
                else
                    wprintf(L"SPECIAL TOKEN %i, breaking", tmp);*/
                break;
            }

            if (beams[0]->lastOutTokenId == special_tokens_start) {
                wprintf(L"EOT TOKEN, breaking\n");
                break;
            }
            ...
            if ((i+1) == groupLogProb) {
                ...
                if (sumLogProb <= groupLogP_thresh) {
                    for(int u = 0; u < beams[0]->probs.size(); u++)
                        wprintf(L"LOW PROB ID %i (%s): %.2f\n", beams[0]->ids[u], mapIdToUTF8[beams[0]->ids[u]].c_str(), beams[0]->probs[u]);
                    wprintf(L"EARLY EXIT DUE TO LOW PROB (hallucinations?) sum=%.2f\n", sumLogProb);
                    beams[0]->ids = {};
                    beams[0]->probs = {};
                    beams[0]->probsTemp = {};
                    break;
            ...
            if(numEqual == 2) {
                ...
                wprintf(L"Detected repeats 2. Setting last token to %i (%s)", altToken, mapIdToUTF8[altToken].c_str());
            }*/
            ...
            if((i+1) >= numRepeatDetection) {
                ...
                if(lastTokensEqual == true) {
                    ...
                    wprintf(L"(NEW) DETECTED REPEATED HALLUCINATION in last few tokens -- Removing and BREAKING\n");
                    break;
                }
            }*/
            ...
            for(int j = 0; j < rankTokens.size(); j++) {
                if(rankTokens[j] == 220) { //isTimestampToken(rankTokens[j])) {
                    ...
                    wprintf(L"(NEW) BLANK TOKEN 220 with rank %i (p=%.2f)", j, p220);
                    break;
                }
            }
            ...
            if(rankTokens[1] == 220) {
                wprintf(L"(NEW) BREAKING DECODING, because 220 has rank 1");
                //beams[0]->ids.pop_back();
                //beams[0]->probs.pop_back();
                //beams[0]->probsTemp.pop_back();
                beams[0]->lastOutTokenId = special_tokens_start;
            ...
            if (beams[0]->lastOutTokenId == 1097) {
                wprintf(L"(NEW) BREAKING DECODING, found \" ...\"\n");
                beams[0]->ids.pop_back();
                beams[0]->probs.pop_back();
                beams[0]->probsTemp.pop_back();
            ...
            if ((i + 1) >= groupLogProb) {
                ...
                if (sumLogProb <= groupLogP_thresh) {
                    //for(int u = 0; u < beams[0]->probs.size(); u++)
                    //    wprintf(L"LOW PROB ID %i (%s): %.2f\n", beams[0]->ids[u], mapIdToUTF8[beams[0]->ids[u]].c_str(), beams[0]->probs[u]);
                    wprintf(L"(NEW) DETECTED LOW PROB in last few tokens -- sum=%.2f\n\n", sumLogProb);
            ...
            if (comprRatio >= 1.75 && hallucLength >= 6 && (promptComprRatio < comprRatio))
            {
                ...
                wprintf(L"DETECTED HIGH COMPRESSION (%.2f AT PEAK INDEX %i, fingerprint %i, strSim %.2f). Rectifying and breaking.\n", comprRatio, peakComprIndex, hallucFingerprint, strSim);
                ...
                if (strSim >= 0.70 && comprRatio < 5.0) {
                    ...
                    wprintf(L"--> HOWEVER, high-compression strings in last 2 iterations are 'equal' (sim=%.2f). FALSELY DETECTED HALLUCINATION. In any case, BREAKING\n", strSim);
            ...
            if (0 == 1 && bestBeamCompressionRatioCheck(*this, prompt, beams[0], peakComprTokenIndex) == false) {
                wprintf(L"BEST BEAM COMPRESSION RATIO IS DANGEROUS, break-ing\n");
                //wprintf(L"(ALSO last token probability = %.2f)", (double)beams[0]->probs[beams[0]->probs.size()-1]);
                ...
                /*if(firstRankToken1Index == -1) {
                    wprintf(L"CAN'T RECTIFY. breaking");
                    beams[0]->ids = {};
                    beams[0]->probs = {};
                    beams[0]->probsTemp = {};
                } else {
                    wprintf(L"ATTEMPTED RECTIFICATION. breaking");
                ...
                wprintf(L"ATTEMPTED RECTIFICATION#2 (peakComprTokenIndex=%i). breaking\n", peakComprTokenIndex);
    ...
    void updateBeams(int beamSize, double temperature)
    {
        ...
        for (int i = 0; i < beams.size(); i++) {
            ...
            for (int j = 0; j < beamSize; j++) {
                ...
                //wprintf(L"updateBeams probability %.2f\n", prob);
    ...
    whisper_mel computeSpectrogramFromBuff(vector<float> buff)
    {
        ...
        //wprintf(L"First mel data is %.2f", (double)mel.data[0]);
        return mel;
    }
